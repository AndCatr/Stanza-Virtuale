...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS stanze (
                    ...
                    numero_eric TEXT,
                    countdown_iniziato INTEGER DEFAULT 0
                 )''')
    conn.commit()
    conn.close()
...
def home():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('stanze.db')
        c = conn.cursor()
        c.execute("INSERT INTO stanze (codice, chat, numero_penelope, numero_eric, countdown_iniziato) VALUES (?, ?, ?, ?, ?)",
                  (codice, "", "", "", 0))
        conn.commit()
        conn.close()
...
def stanza(codice):
    ...
    conn = sqlite3.connect('stanze.db')
    c = conn.cursor()
    c.execute("SELECT chat, numero_penelope, numero_eric, countdown_iniziato FROM stanze WHERE codice = ?", (codice,))
    stanza = c.fetchone()
    conn.close()
    ...
    chat, numero_penelope, numero_eric, countdown_iniziato = stanza
...
def aggiorna_chat(codice):
    ...
    return jsonify({"chat": [msg.split(": ", 1) for msg in stanza[0].split("\n") if ": " in msg]})
...
if __name__ == '__main__':
    app.run(host="0.0.0.0", port=5000, debug=True)  # Forza il riavvio del server con debug attivo
